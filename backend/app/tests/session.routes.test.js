...
// -----------------------------------------------------------
//
// The user row is re-read from the DB by the session's oid,
...
test("check: returns the DB user row plus roles as { name } objects", async () => {
  signInAs("oid-1", ["Darbuotojas", "Vadybininkas"], { email: "j@vu.lt", name: "Jonas" });
  onQuery(/SELECT oid, email, full_name, created_at, last_login_at FROM users WHERE oid = \$1/, [
    { oid: "oid-1", email: "j@vu.lt", full_name: "Jonas Jonaitis" },
  ]);

  const res = await api(app.base, "GET", "/api/session/check");
  assert.equal(res.status, 200);
  assert.equal(res.body.user.oid, "oid-1");
  assert.deepEqual(res.body.roles, [{ name: "Darbuotojas" }, { name: "Vadybininkas" }]);
  assert.deepEqual(queryLog()[0].params, ["oid-1"]);
});
